...
from utils import (BLUE, RED, INK, MUTED, get_theme, inject_theme_css, kpi,
                   load_my_matches, partner_win_rate, render_demo_badge,
                   render_table, rolling_win_rate, section_caption, shot_dna,
                   sidebar_player_name, themed_fig, win_rate_display)
...
with c2:
    n_all = len(mm)
    wins_all = int(mm["won"].sum())
    wr = mm["won"].mean()
    if n_all < 10:
        kpi("WIN RATE", "-",
            f"not enough matches yet for a reliable estimate (n={n_all})")
    else:
        lo, hi = wilson_ci(wins_all, n_all)
        kpi("WIN RATE", f"{wr*100:.0f}%",
            f"CI {lo*100:.0f}-{hi*100:.0f}% · {wins_all}W · {n_all - wins_all}L")
with c3:
    ...
    if n3 == 0:
        ...
        kpi("3-SET FADE", "-",
            f"not enough matches yet for a reliable estimate (n={n3})")
    else:
        wins3 = int(three["won"].sum())
        lo, hi = wilson_ci(wins3, n3)
        rate_three = wins3 / n3 * 100
        rate_two = two["won"].mean() * 100
        kpi("3-SET FADE", f"{rate_three:.0f}%",
            f"CI {lo*100:.0f}-{hi*100:.0f}% · vs {rate_two:.0f}% in 2-set · "
            f"{rate_three - rate_two:+.0f}pp",
...
with r1c2:
    ...
    best_wr = win_rate_display(best["wins"], best["matches"])
    worst_wr = win_rate_display(worst["wins"], worst["matches"])
    section_caption(
        f"<b>{best['partner']}</b> highlighted as the strongest pairing "
        f"({best_wr}); <b>{worst['partner']}</b> as the weakest "
        f"({worst_wr}).")
...
with r2c2:
    ...
    st.plotly_chart(themed_fig(fig), use_container_width=True,
                    config={"displayModeBar": False})
    two_wr = win_rate_display(int(two["won"].sum()), len(two)) if len(two) else "n=0"
    three_wr = win_rate_display(int(three["won"].sum()), len(three)) if len(three) else "n=0"
    section_caption(
