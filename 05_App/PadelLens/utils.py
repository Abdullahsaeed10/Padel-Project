...
import db
from analytics import wilson_ci
...
# ---------------------------------------------------------------------------
# Data loading (cached so reruns don't reread the DB/CSVs).
#
# These are thin wrappers around db.py: db.py owns "where does the data
# actually live" (SQLite if data/padellens.db has been built via build_db.py,
# transparently falling back to the bundled CSVs otherwise); utils.py just
# adds the Streamlit caching layer and the derived columns each page expects.
# ---------------------------------------------------------------------------

@st.cache_data
def load_pro_players() -> pd.DataFrame:
    return db.load_pro_players()


@st.cache_data
def load_pro_matches() -> pd.DataFrame:
    return db.load_pro_matches()


@st.cache_data
def load_my_matches(user_id: int = db.DEMO_USER_ID) -> pd.DataFrame:
    df = db.load_my_matches(user_id=user_id)
    df["won"] = df["result"] == "W"
...
def pro_data_meta() -> dict:
    """See db.pro_data_meta — surfaced here so pages only need one import line."""
    return db.pro_data_meta()


def pro_data_caption():
    """Render the required per-pro-page caption: real 2026 feed vs demo dataset."""
    st.caption(pro_data_meta()["caption"])


def win_rate_display(wins: int, n: int) -> str:
    """Format a win rate with its Wilson 95% CI, e.g. '62% (CI 45-77, n=24)'.

    Below n=10 the sample is too small to support a point estimate at all
    (this is the same threshold the Insights page's method notes use), so this
    returns a plain-language caveat instead of a percentage — an
    academic-honesty requirement: never state a bold win-rate claim on a
    handful of matches.
    """
    if n < 10:
        return f"not enough matches yet for a reliable estimate (n={n})"
    lo, hi = wilson_ci(int(wins), int(n))
    rate = wins / n
    return f"{rate*100:.0f}% (CI {lo*100:.0f}-{hi*100:.0f}, n={n})"


def render_demo_badge():
    """Visible 'SYNTHETIC DEMO DATA' badge for the personal match-log pages.

    Academic-honesty requirement: the bundled my_matches.csv is a synthetic
    seed for demo user 1 ("Demo User"), not a real player's logged history, so
    every page built on it must say so clearly rather than let it read as real
    personal data. db.is_demo_user() checks the users table (or defaults to
    True before data/padellens.db has been built, since the CSV fallback IS
    that seed).
    """
